...
export default function Game3AntiTreasure({ durationMs, onEvent, onDone }: Props){
  ...
  useEffect(() => {
    ...
    const newTrial = () => {
      ...
      onEvent('trial_start', { game:3, trial_id:id, cueSide })
    }
    ...
    const draw = () => {
      ...
      if (phase==='fix' && t >= nextAt){
        phase='cue'
        current!.tCue = t
        onEvent('cue_on', { game:3, trial_id:current!.id, side: current!.cueSide })
        nextAt = t + 380
      }
      ...
      if (phase==='cue'){
        ...
        if (t >= nextAt){
          ...
          onEvent('target_on', { game:3, trial_id:current!.id, side: targetSide, x:p.x, y:p.y })
          nextAt = t + 1200
        }
      }
      ...
      if (phase==='resp'){
        ...
        // timeout auto next
        if (t >= nextAt){
          onEvent('trial_end', { game:3, trial_id:current!.id, reason:'timeout', correct:0 })
          newTrial()
        }
      }
      ...
      ctx.fillText(`Coins: ${coins}`, 18, 55)

      if (elapsed >= durationMs){
        onEvent('game_end', { game: 3, coins })
        setDone(true)
        onDone()
        return
      }
    ...
    const onClick = (e: MouseEvent) => {
      ...
      if (phase==='resp'){
        ...
        if (dT <= HIT_R){
          setCoins(v => v + 1)
          onEvent('response', { game:3, trial_id:current.id, correct:1, rt_ms:rt })
          onEvent('trial_end', { game:3, trial_id:current.id, reason:'click', correct:1 })
          newTrial()
        } else if (dAlien <= HIT_R){
          setBlastOn(true)
          setTimeout(()=>setBlastOn(false), 450)
          onEvent('response', { game:3, trial_id:current.id, correct:0, rt_ms:rt, clickedAlien:true })
          onEvent('trial_end', { game:3, trial_id:current.id, reason:'clicked_alien', correct:0 })
          newTrial()
        }
      }
    }
  ...
  }, [canvasRef, bg, rover, alien, treasure, blast, durationMs, coins, onEvent])
